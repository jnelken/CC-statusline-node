...
- 🖥️ **Node·Nerd Font 불필요** — 순수 Bash + 표준 이모지, 설치 직후 바로 제대로 보임.
- 📦 **의존성 자동 설치** — macOS·Linux·Windows에서 `jq`(윈도우는 Git Bash까지) 자동 설치.
...
**상태줄이 비어 보이거나, 빈 터미널 창이 계속 떠요 (Windows).** `./install.ps1`을 다시 실행하세요 — 이제 스크립트를 `bash`로 호출해(`.sh` 파일 연결로 창이 뜨지 않게) 실행하고, `jq`를 스크립트 옆에 함께 복사합니다(`jq`가 PATH에 없어도 동작). 자세히: [TROUBLESHOOTING.md](TROUBLESHOOTING.md).
